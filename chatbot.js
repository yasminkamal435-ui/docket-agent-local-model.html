 ...
 * Qwen2.5-0.5B-Instruct (ONNX, via transformers.js). Chosen deliberately
 * over the larger 1.5B variant: the 1.5B model (~1GB) was too heavy to
 * reliably finish downloading on slower/mobile connections, which made
 * the assistant *look* broken/stuck even though it would eventually work.
 * 0.5B (~300-400MB quantized) loads far more reliably while still being a
 * genuine, modern instruction-tuned model — noticeably better than the
 * original 135M model this project started with.
 *
