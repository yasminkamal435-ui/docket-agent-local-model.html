...
export async function renderDashboard(lang, d) {
  ...
  charts.cityChart = new window.Chart(document.getElementById('cityChart'), {
    type: 'bar',
    data: { labels: cities, datasets: [{ label: d.dashboard.priceByCity, data: pricePerSqmByCity, backgroundColor: '#1E8449' }] },
  ...
  charts.finishChart = new window.Chart(document.getElementById('finishChart'), {
    type: 'doughnut',
    data: { labels: finishes, datasets: [{ data: priceByFinish, backgroundColor: ['#1E8449', '#3C9F5F', '#C97A1A', '#B5652B', '#4C5D74', '#7A889C'] }] },
  ...
  charts.areaChart = new window.Chart(document.getElementById('areaChart'), {
    type: 'line',
    data: { labels: areaPoints.map(a => a + ' m²'), datasets: [{ label: d.dashboard.distribution, data: areaPrices, borderColor: '#1E8449', tension: 0.3, fill: false }] },
    options: { plugins: { legend: { display: false } }, responsive: true, maintainAspectRatio: false },
  });
  ...
  charts.importanceChart = new window.Chart(document.getElementById('importanceChart'), {
    ...
    data: {
      ...
      datasets: [{ label: d.dashboard.featureImportance, data: importance.map(i => Math.round(i.magnitude)), backgroundColor: '#14532D' }],
    },
